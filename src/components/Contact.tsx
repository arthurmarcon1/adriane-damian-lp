...
export function Contact() {
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    console.log("Mensagem enviada:", Object.fromEntries(data));
    setSent(true);
  }
      ...
      <div className="max-w-7xl mx-auto px-8 md:px-16 grid md:grid-cols-12 gap-12 md:gap-16">
        ...
        <div className="md:col-span-5">
          ...
          <p className="font-sans font-light text-base text-bg-secondary/80 leading-relaxed mb-12">
            O primeiro contato é confidencial e sem compromisso. Apresente sua situação
            pelos canais abaixo e agendaremos uma análise adequada.
          </p>
          ...
          <div className="bg-graphite-darker p-10 md:p-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input id="name" name="name" type="text" required placeholder="Seu nome completo" className={fieldCls} />
              </div>
              <div>
                <label className={labelCls} htmlFor="email">E-mail</label>
                <input id="email" name="email" type="email" required placeholder="seu@email.com" className={fieldCls} />
              </div>
              <div>
                <label className={labelCls} htmlFor="phone">Telefone</label>
                <input id="phone" name="phone" type="tel" placeholder="(00) 00000-0000" className={fieldCls} />
              </div>
              <div>
                <label className={labelCls} htmlFor="subject">Assunto</label>
                <select id="subject" name="subject" className={fieldCls} defaultValue="">
                  <option value="" disabled className="bg-graphite-darker">Selecione</option>
                  <option className="bg-graphite-darker">Direito Civil</option>
                  <option className="bg-graphite-darker">Direito Criminal</option>
                  <option className="bg-graphite-darker">Direito Empresarial</option>
                  <option className="bg-graphite-darker">Direito de Família</option>
                  <option className="bg-graphite-darker">Outro</option>
                </select>
              </div>
              <div>
                <label className={labelCls} htmlFor="message">Mensagem</label>
                <textarea id="message" name="message" rows={4} placeholder="Descreva brevemente sua situação..." className={fieldCls} />
              </div>

              <div className="mt-10">
                <button
                  type="submit"
                  className="w-full md:w-auto bg-gold-champagne text-graphite-deep hover:bg-bg-primary transition-colors px-10 py-4 font-sans font-medium text-sm tracking-[0.2em] uppercase rounded-none"
                >
